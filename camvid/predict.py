...
images_path = "data1/dataset1/images_prepped_test/"
segs_path = "data1/dataset1/annotations_prepped_test/"
...
print(images,segmentations)
# m = load_model("output/%s_model.h5" % key)
model = method[key](11,320,320) # 有自定义层，不能直接加载模型
model = load_model("output/%s_model.h5" % key)
...
for i, (imgName, segName) in enumerate(zip(images, segmentations)):
    ...
    #print(np.expand_dims(LoadBatches.getImageArr(im), 0))
    #print(m)
